  ...
  useEffect(() => {
    if (selectedPayment) {
      ...
      months.forEach((month) => {
        const isPaid =
          selectedPayment.monthlyFee[
            month.key as keyof typeof selectedPayment.monthlyFee
          ];
        if (!isPaid) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updatePayment.mutateAsync({
        ...
        months: months
          .filter((month) => !selectedMonths.includes(month.number.toString()))
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {selectedPayment.className} - {selectedPayment.year}
          </p>
        </div>
