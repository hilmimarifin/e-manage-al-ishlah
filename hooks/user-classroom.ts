...
export function useAddStudentToClass() {
    ...
    return useMutation({
      ...
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['classrooms'] })
        showToast.success('Student added to class successfully', 'Student has been added to the class')
      },
      onError: (error: any) => {
        showToast.error('Failed to add student to class', error.message || 'Please try again')
      }
    })
  }
...
export function useUpdateClassroom() {
  ...
  return useMutation({
    mutationFn: ({ id, ...data }: any) => apiClient.put(`/classrooms/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['classrooms'] })
